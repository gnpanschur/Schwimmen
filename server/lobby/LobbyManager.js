...
class LobbyManager {
  ...
  joinRoom(socketId, roomCode, playerName, autoReady = false) {
    ...
    const result = room.addPlayer(socketId, playerName, autoReady);
    if (result.success) {
      this.socketToRoomMap.set(socketId, code);
      console.log(`[LobbyManager] Player ${playerName} (${socketId}) joined room ${code} (autoReady: ${autoReady})`);
      return {
        success: true,
        roomCode: code,
        state: room.getClientState()
  ...
  attachSocketListeners(socket) {
    ...
    socket.on(LOBBY_EVENTS.JOIN_ROOM, ({ roomCode, playerName, autoReady }, callback) => {
      const res = this.joinRoom(socket.id, roomCode, playerName, autoReady);
      if (res.success) {
        socket.join(res.roomCode);
        this.broadcastRoomState(res.roomCode);
      }
